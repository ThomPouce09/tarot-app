...
export default function YiJingQuestionInterpretationPage() {
  ...
  useEffect(() => {
    ...
    const baguetteNum = localStorage.getItem('yi-jing-question-baguette');
    const question = localStorage.getItem('yi-jing-question-text');
    ...
    fetch('/api/yi-jing-question-interpretation', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ baguette: Number(baguetteNum), userId, question: question || undefined })
    })
      .then(res => res.json())
      .then(data => {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-black">
        <p className="text-yellow-400 text-lg animate-pulse">L'oracle consulte les hexagrammes...</p>
      </div>
    );
  }
        ...
        <div className="h-full overflow-y-auto scrollbar-hide">
          {error ? (
            <>
              ...
              <Link href="/yi-jing-question" className="text-yellow-400 underline">
                Retour au tirage
              </Link>
            </>
          ) : interpretation ? (
            <>
              ...
              {interpretation.question && (
                <div className="mb-4 bg-yellow-950/20 border border-yellow-700/30 rounded-lg p-3">
                  ...
                  <p className="text-yellow-200 italic text-sm">"{interpretation.question}"</p>
                </div>
              )}
